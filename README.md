...
| **输出格式（通用 MP4）** | 默认输出 **H.264 视频 + AAC 音频的 MP4**，Windows 默认「电影和电视」、各类播放器均可直接打开：选流时优先挑 AVC(H.264) 视频 + AAC 音频并直接封装（秒级、不重编码）；仅少数只有 AV1/HEVC 或 E-AC3/FLAC 的极清档才会触发 ffmpeg 重编码兜底 |
